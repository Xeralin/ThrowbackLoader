...
constexpr uint32_t SAVE_HEADER_OFFSET = 0x28;
...
[[nodiscard]] std::optional<uint64_t> get_file_size(const std::filesystem::path& file_path)
{
    std::error_code error_code;
...
DLLEXPORT int UPLAY_SAVE_GetSavegames(UplayList** out_games_list, UplayOverlapped* overlapped)
{
    ...
    for (const auto& save_path : savefile_paths.value())
    {
        ...
        save_file.seekg(SAVE_HEADER_OFFSET, std::ios::beg);

        auto data_buffer = std::unique_ptr<char[]>(new (std::nothrow) char[SAVE_HEADER_SIZE]);
        if (!data_buffer || !save_file.read(data_buffer.get(), SAVE_HEADER_SIZE))
        {
            continue;
        }
        ...
        file_list_item->num = static_cast<DWORD>(save_handle.value());
                              ...
                              uint8_t** out_data, uint32_t* out_bytes_read, UplayOverlapped* overlapped)
{
    ...
    const auto file_size_result = get_file_size(file_path);
    if (!file_size_result.has_value() || *file_size_result < SAVE_PADDING_SIZE)
    {
        return 0;
    }
    const uint64_t file_size = *file_size_result;
